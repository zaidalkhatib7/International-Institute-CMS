...
const copyByLanguage = {
  ar: {
    ...
    pathway: 'المسار', standard: 'المعيار', level: 'المستوى', slug: 'المعرف', rank: 'الرتبة', version: 'النسخة', order: 'الترتيب', behavior: 'السلوك', quorum: 'النصاب', quorumSingleOnly: 'النصاب مثبت على 1: النصاب الأكبر يتطلب تسجيل أصوات متعددة، وشاشة التصويت غير مبنية بعد، ولا يوجد إلغاء — فتُحبس الحالة نهائيًا.', amount: 'المبلغ', currency: 'العملة', effectiveFrom: 'ساري من', effectiveUntil: 'ساري حتى',
    ...
    geminiTitle: 'مساعد Gemini لتقييم RPL', geminiHint: 'تحت سيطرة المدير فقط. يستقبل نموذج التقييم وبيانات الأدلة دون الملفات الخام؛ ولا يستطيع التحقق من الأدلة أو اعتماد النتيجة أو إصدار قرار اللجنة.', geminiModel: 'نموذج Gemini', geminiPrompt: 'Prompt التقييم الاستشاري', geminiGovernance: 'يُحفظ الـPrompt ومصدر الحقيقة والمدخلات والمخرجات مع كل مسودة. راجعه ضمن حوكمة RPL.',
  },
  en: {
    ...
    pathway: 'Pathway', standard: 'Standard', level: 'Level', slug: 'Slug', rank: 'Rank', version: 'Version', order: 'Sort order', behavior: 'Behaviour', quorum: 'Quorum', quorumSingleOnly: 'Quorum is fixed at 1: a higher quorum needs multiple recorded votes, the vote panel is not built yet, and there is no cancel — the case would be stuck permanently.', amount: 'Amount', currency: 'Currency', effectiveFrom: 'Effective from', effectiveUntil: 'Effective until',
    ...
    geminiTitle: 'Gemini RPL evaluation assistant', geminiHint: 'Administrator-controlled only. It receives the rubric and evidence metadata, never raw uploads; it cannot verify evidence, adopt an outcome, or make a committee decision.', geminiModel: 'Gemini model', geminiPrompt: 'Advisory evaluation prompt', geminiGovernance: 'The prompt, source of truth, input, and output are snapshotted with every draft. Review them through RPL governance.',
  },
  nl: {
    ...
    pathway: 'Traject', standard: 'Norm', level: 'Niveau', slug: 'Slug', rank: 'Rang', version: 'Versie', order: 'Volgorde', behavior: 'Gedrag', quorum: 'Quorum', quorumSingleOnly: 'Quorum is fixed at 1: a higher quorum needs multiple recorded votes, the vote panel is not built yet, and there is no cancel — the case would be stuck permanently.', amount: 'Bedrag', currency: 'Valuta', effectiveFrom: 'Geldig vanaf', effectiveUntil: 'Geldig tot',
    ...
    geminiTitle: 'Gemini-assistent voor RPL-beoordeling', geminiHint: 'Uitsluitend onder beheer van een beheerder. Ontvangt rubric- en bewijsmetadata, nooit ruwe uploads; kan geen bewijs verifiëren, uitkomst vaststellen of commissiebesluit nemen.', geminiModel: 'Gemini-model', geminiPrompt: 'Prompt voor adviserend concept', geminiGovernance: 'Prompt, bron van waarheid, invoer en uitvoer worden bij elk concept vastgelegd. Laat deze via RPL-governance beoordelen.',
  },
}
...
export default function RplConfigurationPage() {
  ...
  const [settings, setSettings] = useState({ appeal_window_days: 15, require_verified_evidence_for_assessment: true, gemini_advisory_model: 'gemini-3.5-flash', gemini_advisory_prompt: '' })
  ...
  function resourceFields() {
    ...
    if (activeTab === 'committees') return <><Input type="number" min="1" max="1" label={copy.quorum} value={form.quorum} onChange={(event) => setForm((current) => ({ ...current, quorum: event.target.value }))} /><p className="text-xs text-amber-800">{copy.quorumSingleOnly}</p></>
