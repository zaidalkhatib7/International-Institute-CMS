...
export default function RplConfigurationPage() {
  ...
  const [settings, setSettings] = useState({ appeal_window_days: 15, require_verified_evidence_for_assessment: true })
  ...
  function resourceFields() {
    ...
    if (activeTab === 'outcomes' || activeTab === 'actions') return <Select label={copy.behavior} value={form.behavior} onChange={(event) => setForm((current) => ({ ...current, behavior: event.target.value }))}>{(activeTab === 'outcomes' ? ['achieved', 'development_required', 'limited_completion', 'not_achieved'] : ['approve', 'conditional', 'remediation', 'reject', 'reassign']).map((value) => <option key={value} value={value}>{value.replaceAll('_', ' ')}</option>)}</Select>
