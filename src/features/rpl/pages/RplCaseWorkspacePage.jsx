...
const copyByLanguage = {
  ...
  en: {
    ...
    assessments: 'Assessment reports', noAssessments: 'No assessment sessions exist for this case yet.', openAssessment: 'Open assessment',
  ...
  nl: {
    ...
    assessments: 'Beoordelingsrapporten', noAssessments: 'Er zijn nog geen beoordelingen voor dit dossier.', openAssessment: 'Beoordeling openen',
...
export default function RplCaseWorkspacePage() {
  ...
  const completeness = readApplicationCompleteness(application)?.percentage ?? 0
  ...
  function renderOverview() {
    return (
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.65fr)]">
          ...
          <Card>
            ...
            <CardContent className="pt-6">
              <dl className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <Definition label={copy.submitted} value={application.submitted_at ? new Date(application.submitted_at).toLocaleString(language === 'ar' ? 'ar' : language) : '—'} />
              </dl>
            </CardContent>
          </Card>
          <Card>
        ...
        <div className="space-y-6">
          <CompletenessMeter value={completeness} language={language} />
          <Card>
        ...
        <Card>
          ...
          <CardContent className="pt-6">
            <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_220px_auto] lg:items-end">
              ...
              <Select label={copy.role} value={assignment.assignment_role} onChange={(event) => setAssignment((current) => ({ ...current, assignment_role: event.target.value }))}><option value="primary">Primary</option><option value="secondary">Secondary</option></Select>
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {state.assignments.length ? state.assignments.map((item) => <div key={item.id} className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface-muted)] p-4"><div className="flex items-center justify-between gap-3"><span className="font-bold text-[var(--color-text)]">{item.expert?.user?.name || item.assessor?.user?.name || item.assessor_name}</span><Badge variant={item.conflict_declaration?.resolution === 'cleared' ? 'success' : 'warning'}>{item.status || item.assignment_role}</Badge></div><p className="mt-2 text-sm text-[var(--color-text-muted)]">{item.assignment_role || '—'}</p><div className="mt-4 flex flex-wrap gap-2">{item.status === 'pending' ? <Button size="sm" variant="outline" onClick={() => acceptAssignment(item.id)}>{copy.acceptAssignment}</Button> : null}{item.status === 'accepted_pending_conflict' ? <><Button size="sm" variant="outline" onClick={() => declareNoConflict(item.id)}>{copy.noConflict}</Button><Button size="sm" variant="ghost" onClick={() => setConflictDialog({ open: true, assignmentId: item.id, details: '' })}>{copy.reportConflict}</Button></> : null}{item.status === 'accepted' && !item.assessment ? <Button size="sm" variant="outline" onClick={() => startAssessment(item.id)}>{copy.startAssessment}</Button> : null}</div></div>) : <p className="text-sm text-[var(--color-text-muted)]">{copy.noAssignments}</p>}
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="border-b border-[var(--color-border)]"><CardTitle>{copy.assessments}</CardTitle></CardHeader>
          <CardContent className="space-y-4 pt-6">{state.assessments.length ? state.assessments.map((item) => <article key={item.id} className="flex flex-col gap-4 rounded-2xl border border-[var(--color-border)] p-5 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-bold">{item.reference_number || `Assessment #${item.id}`}</h3><RplStatusBadge status={item.status} language={language} /></div><p className="mt-2 text-sm text-[var(--color-text-muted)]">{item.assignment?.expert?.user?.name || item.assessor_name || '—'}</p></div><Button variant="outline" onClick={() => navigate(`/rpl/assessments/${item.id}`)}>{copy.openAssessment}</Button></article>) : <p className="py-8 text-center text-[var(--color-text-muted)]">{copy.noAssessments}</p>}</CardContent>
        </Card>
      </div>
    )
  }
    ...
    return (
      <div className="grid gap-6 xl:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent className="space-y-4 pt-6">
            <Select label={copy.qualityResult} value={quality.decision} onChange={(event) => setQuality((current) => ({ ...current, decision: event.target.value }))}><option value="approved">{copy.approve}</option><option value="returned">{copy.return}</option><option value="rejected">Rejected</option></Select>
